...
**기업순위**
- 네이버 증권은 '시가총액'을 기준으로 기업순위를 제공 (시가총액 기준 조회 시 함께 제공되는 경제지표 확인)
- ROE, PER 등 지표로 정렬해서 확인하는 기능이 없다.
- 키움증권 API를 확인했을 때 이에 부합하는 API가 제공되지 않음
- '종목정보 리스트' API와 '주식 기본정보 요청' API를 활용하여 종목별 정보를 받아 DB에 저장한 뒤 검색.

### 자산관리

**실현손익**
- 증권사에서 조회할 수 있는 실현손익은 기간이 제한되어 있기 때문에 원하는 기간의 손익을 확인하기 위한 기능 추가.
