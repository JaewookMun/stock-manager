...
# Build (Windows - use gradlew.bat or .\gradlew) — always skip tests
...
|--------|------|-------------|
| GET | `/api/accounts` | 계좌번호 목록 조회 |
| GET | `/api/assets/realized-pnl` | 실현손익 기록 조회 (기간) |
| GET | `/api/assets/cash-flow` | 이체내역 조회 (현금흐름) |
| GET | `/api/stocks/conditions` | 검색조건 목록 조회 |
| GET | `/api/stocks/screen` | 상장기업 조건 검색 |
...
│   ├── AssetController.java       # GET /api/assets/realized-pnl, /cash-flow
│   └── StockController.java       # GET /api/stocks/conditions, /screen
...
│       │   └── stock/             # Stock-specific Kiwoom DTOs (ka10099, ka10001)
...
2. Calls `ka10001` for each stock's financial details, rate-limited at 5 req/s via `ScheduledExecutorService` (200ms intervals between scheduled tasks)
3. `Stock` rows: insert-only for new codes (existing rows not updated). `StockDetail` rows: all new rows inserted each run (no delete-first)
...
- `Account` stores Kiwoom API credentials (`appkey`, `secretkey`) per account; transaction entities (`RealizedPnl`, `CashFlow`) reference accounts by `accountId` (Long FK), mapped via `@ManyToOne(fetch = LAZY)`
